...
class EditorialCanvas(canvas.Canvas):
    ...
    def draw_decorations(self, page_count):
        ...
        page_bookmarks = {
            ...
            4: ("Tablero Integral de Indicadores Líderes", "sec_dashboard"),
            5: ("1. Nivel de Actividad General (EMAE)", "sec_emae"),
            6: ("2. Precios y Salarios (INDEC)", "sec_prices"),
            7: ("Cuadro 1. Aperturas IPC y Canastas", "sec_tab_ipc"),
            8: ("3. Producción Sectorial en Cuyo", "sec_cuyo"),
            9: ("3.1 Comparativo Regional ISARC", "sec_regional_cuyo"),
            10: ("4. Balance BCRA y Postura Monetaria", "sec_monetary"),
            11: ("5. Arbitraje en ARS y Breakeven", "sec_tactical"),
            12: ("6. Curva Soberana Nelson-Siegel", "sec_yield"),
            13: ("7. Microestructura FX y Rofex", "sec_fx"),
            14: ("7.1. Tipo de Cambio Real Bilateral", "sec_tcr"),
            15: ("8. Renta Variable y Balances", "sec_equity"),
            16: ("9. Flash Normativo y Referencias", "sec_refs")
        }
        ...
        if self._pageNumber > 1:
            ...
            # Páginas 4 a 9: Bloque I: 'ACTIVIDAD REAL & REGIONAL' (#047857)
            # Páginas 10 a 14: Bloque II: 'MONEDA, TASAS & DEUDA' (#0284C7)
            # Página 15: Bloque III: 'RENTA VARIABLE & ALLOCATION' (#9A3412)
            # Página 16: Bloque IV: 'METODOLOGÍA & GOBERNANZA' (#475569)
            if 4 <= p <= 9:
...
def crear_pagina_arquetipo_desglose(
    ...
    tbl_pad = 1.8 if len(tabla_data) >= 12 else (2.2 if len(tabla_data) > 10 else 2.6)
    t_ind = Table(tabla_data, colWidths=tabla_col_widths)
    t_ind.setStyle(TableStyle([
        ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
        ('TOPPADDING', (0,0), (-1,-1), tbl_pad),
...
def crear_pagina_tablero_integral_macro(ctx):
    """
    Arquetipo F: Tablero Integral de Indicadores Macroeconómicos Líderes & Desagregaciones Estructurales (Página 4).
    Consolida las 4 dimensiones críticas del programa de estabilización:
    1. Actividad Real & Desagregación Sectorial (INDEC EMAE, ISAC, IPI, Agro, Minería/Petróleo, Comercio, FBCF).
    2. Precios Relativos, Formación Mayorista & Canastas (IPC General, Núcleo, Regulados, IPIM General, IPIM Importados, CBT, CBA).
    3. Sector Externo, Balanza de Pagos & Divisas (Cuenta Corriente, Balanza Comercial FOB, Servicios CIF, Cta Financiera, RIN BCRA, TCRM, TCR Brasil).
    4. Confianza, Expectativas & Mercados Financieros (ICG UTDT, ICC UTDT, REM BCRA, Dólar CCL & Brecha, EMBI+, Lecap TEM, Cobertura Salario RIPTE/CBT).
    """
    flowables = []
    
    flowables.append(Paragraph(
        "<font color='#047857' size=7.8><b>SÍNTESIS MACROECONÓMICA INTEGRAL · TABLERO CUANTITATIVO DE INDICADORES LÍDERES</b></font>",
        ParagraphStyle('Kicker_Dash', parent=styles['Normal'], fontName='Palatino-Bold', leading=9.6, spaceAfter=2)
    ))
    flowables.append(Paragraph("Tablero Integral de Indicadores Macroeconómicos Líderes & Desagregaciones", title_style))
    
    lead_txt = (
        "Matriz analítica de seguimiento integral que articula las cuatro dimensiones críticas del programa de estabilización: "
        "actividad económica desagregada por sectores transables y no transables, dinámica de precios relativos y costos mayoristas (IPIM), "
        "solvencia externa en balanza de pagos y reservas internacionales netas, y barómetro de expectativas, confianza (ICG/ICC) y mercado financiero."
    )
    t_lead = Table([[Paragraph(lead_txt, leadin_style)]], colWidths=[532])
    t_lead.setStyle(TableStyle([
        ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
        ('LEFTPADDING', (0,0), (-1,-1), 8),
        ('RIGHTPADDING', (0,0), (-1,-1), 6),
    ...
    act = ctx.get("actividad", {})
    inf = ctx.get("inflacion", {})
    ext = ctx.get("sector_externo", {})
    conf = ctx.get("confianza", {})
    dol = ctx.get("dolar", {})
    tas = ctx.get("tasas_bcra", {})
    
    emae_ia = act.get("emae_interanual_pct", 3.1)
    emae_mom = act.get("emae_desestacionalizado_mom_pct", 0.6)
    isac_ia = act.get("isac_construccion_ia_pct", -4.2)
    isac_mom = act.get("isac_construccion_mom_pct", 1.4)
    ipi_ia = act.get("ipi_manufacturero_ia_pct", -1.8)
    ipi_mom = act.get("ipi_manufacturero_mom_pct", 0.8)
    
    ipc_gral = inf.get("indec_general_mom", 2.2)
    ipc_core = inf.get("indec_nucleo_mom", 1.9)
    ipim_mom = inf.get("ipim_mayorista_mom", 1.8)
    ipim_ia = inf.get("ipim_mayorista_ia_pct", 28.5)
    ipim_imp = inf.get("ipim_importados_mom", 1.2)
    cbt_mza = inf.get("canasta_basica_total_mza", 963000)
    
    cc_usd = ext.get("cuenta_corriente_usd_m", 1840)
    bc_usd = ext.get("balanza_comercial_bienes_usd_m", 3250)
    exp_usd = ext.get("exportaciones_bienes_fob_usd_m", 7120)
    imp_usd = ext.get("importaciones_bienes_cif_usd_m", 3870)
    rin_var = ext.get("variacion_rin_bcra_usd_m", 3650)
    rin_stock = ext.get("reservas_netas_bcra_usd_m", 1240)
    tcrm = ext.get("tcrm_indice", 94.5)
    
    icg = conf.get("icg_utdt", 2.48)
    icc = conf.get("icc_utdt", 41.8)
    ccl = dol.get("ccl", 1600.20)
    brecha = dol.get("brecha_ccl_oficial_pct", 4.52)
    embi = tas.get("riesgo_pais_embi", 506)
    lecap = tas.get("lecap_corta_tem", 2.95)
    rem = inf.get("rem_inflacion_12m", 2.0)
    
    # Estilos compactos para tabla maestra de 6 columnas
    s_th = ParagraphStyle('DTH', fontName='Sans-Bold', fontSize=6.8, leading=8.4, alignment=TA_CENTER, textColor=colors.white)
    s_subhdr = ParagraphStyle('DSH', fontName='Sans-Bold', fontSize=6.8, leading=8.4, textColor=PRIMARY)
    s_lbl = ParagraphStyle('DLB', fontName='Sans-Bold', fontSize=6.6, leading=8.2, textColor=DARK_TEXT)
    s_sublbl = ParagraphStyle('DSLB', fontName='Sans', fontSize=6.5, leading=8.0, textColor=DARK_TEXT)
    s_base = ParagraphStyle('DBS', fontName='Sans', fontSize=6.5, leading=8.0, alignment=TA_CENTER, textColor=MUTED)
    ...
    master_table_data = [
        # Cabecera
        [
            Paragraph("<b>VARIABLE / AGREGADO MACROECONÓMICO</b>", s_th),
            Paragraph("<b>BASE / FUENTE</b>", s_th),
    ...
    t_master.setStyle(TableStyle([
        ...
        ('SPAN', (0,15), (-1,15)),
        ('BACKGROUND', (0,22), (-1,22), colors.HexColor("#E2E8F0")),
        ('SPAN', (0,22), (-1,22)),
        # Lines and borders
    ...
    flowables.append(t_master)
    flowables.append(Spacer(1, 4))
    
    # Dos tarjetas de síntesis analítica (2 columnas de 258 pt)
    card_l_rows = [
        [Paragraph("<b>SÍNTESIS DE ACTIVIDAD & FORMACIÓN DE COSTOS</b>", ParagraphStyle('CDH1', fontName='Palatino-Bold', fontSize=7.0, leading=8.6, textColor=PRIMARY))],
        [Paragraph(
            f"{BULLET_TRIANGLE} <b>Tracción Transable:</b> El EMAE (+3,1% i.a.) encuentra sus principales motores en el agro (+14,2%) y la minería/hidrocarburos (+8,5%), compensando la transición en industria (-1,8%) y construcción (-4,2%), las cuales ya exhiben variaciones mensuales positivas.<br/>"
            f"{BULLET_TRIANGLE} <b>Transmisión Mayorista:</b> El IPIM (+1,8% m/m y 28,5% i.a.) confirma la compresión de márgenes e insumos, eliminando presiones de costos aguas arriba y asegurando la convergencia desinflacionaria.",
            ParagraphStyle('CDB1', fontName='Palatino', fontSize=6.5, leading=8.4, textColor=DARK_TEXT)
        )]
    ]
    card_r_rows = [
        [Paragraph("<b>SOLVENCIA EXTERNA, CONFIANZA & MERCADOS</b>", ParagraphStyle('CDH2', fontName='Palatino-Bold', fontSize=7.0, leading=8.6, textColor=PRIMARY))],
        [Paragraph(
            f"{BULLET_TRIANGLE} <b>Superávit Gemelo & Reservas:</b> La Cuenta Corriente (+USD {fmt_num(cc_usd, 0)}M) y la balanza de bienes (+USD {fmt_num(bc_usd, 0)}M) respaldan la reconstitución de reservas netas (+USD {fmt_num(rin_var, 0)}M), alejando riesgos de estrangulamiento externo.<br/>"
            f"{BULLET_TRIANGLE} <b>Expectativas y Riesgo País:</b> El avance del ICG (2,48 pts) y del ICC (41,8 pts) acompaña la compresión del EMBI+ ({fmt_num(embi, 0)} pb), consolidando tasas reales en pesos e impulsando el financiamiento.",
            ParagraphStyle('CDB2', fontName='Palatino', fontSize=6.5, leading=8.4, textColor=DARK_TEXT)
        )]
    ]
    sub_l = Table(card_l_rows, colWidths=[258])
    sub_l.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#F8FAFC")),
        ('BOX', (0,0), (-1,-1), 0.5, BORDER),
        ('LINELEFT', (0,0), (0,-1), 2.5, PRIMARY),
        ('TOPPADDING', (0,0), (-1,-1), 2.5),
    ...
    sub_r = Table(card_r_rows, colWidths=[258])
    sub_r.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,-1), colors.HexColor("#F8FAFC")),
        ('BOX', (0,0), (-1,-1), 0.5, BORDER),
        ('LINELEFT', (0,0), (0,-1), 2.5, PRIMARY),
        ('TOPPADDING', (0,0), (-1,-1), 2.5),
    ...
    t_cards = Table([[sub_l, "", sub_r]], colWidths=[258, 16, 258])
    ...
    flowables.append(t_cards)
    flowables.append(Spacer(1, 3))
    
    # Footnote institucional
    f_txt = (
        "<b>Nota de Gobernanza Cuantitativa:</b> Metodología de agregación y seguimiento continuo de coyuntura macroeconómica. "
        "Fuentes oficiales primarias: INDEC (EMAE, ISAC, IPI Manufacturero, IPC Nacional, IPIM Mayoristas), Banco Central de la República Argentina (BCRA), "
        "DEIE Mendoza, Universidad Torcuato Di Tella (Centro de Investigación en Finanzas - Poliarquía), Secretaría de Energía de la Nación y ByMA. "
        "Modelado y cálculo de variaciones: Federico Agustín Chillón · Facultad de Ciencias Económicas, Universidad Nacional de Cuyo (UNCUYO)."
    )
    flowables.append(Paragraph(f_txt, footnote_table_style))
    flowables.append(PageBreak())
    return flowables
...
def generar_informe_mensual_reportlab(ctx=None):
    ...
    toc_entries = [
        ("CAT", "ESTRATEGIA & INDICADORES LÍDERES", "", ""),
        ("MAIN", "Resumen Ejecutivo, Matriz de Escenarios y Asignación de Carteras", "3", "sec_exec"),
        ("MAIN", "Tablero Integral de Indicadores Macroeconómicos Líderes", "4", "sec_dashboard"),
        ("CAT", "ECONOMÍA REAL", "", ""),
        ("MAIN", "1. Nivel de Actividad Económica General (EMAE)", "5", "sec_emae"),
        ("MAIN", "2. Dinámica de Precios, Canastas Básicas y Salario Real", "6", "sec_prices"),
        ("SUB", "Cuadro 1. Índice de Precios al Consumidor y Canastas Básicas (INDEC / DEIE)", "7", "sec_tab_ipc"),
        ("MAIN", "3. Desagregación Sectorial y Producción en Mendoza y Cuyo", "8", "sec_cuyo"),
        ("SUB", "3.1. Comparativo Regional: Índice Sintético de Actividad (ISARC Cuyo)", "9", "sec_regional_cuyo"),
        ("CAT", "RÉGIMEN MONETARIO Y MERCADOS", "", ""),
        ("MAIN", "4. Balance del BCRA, Pasivos Cuasifiscales y Postura Monetaria", "10", "sec_monetary"),
        ("MAIN", "5. Arbitraje de Tasas en ARS, Breakeven y Recomendaciones de Cartera", "11", "sec_tactical"),
        ("MAIN", "6. Estructura Temporal de la Deuda Soberana y Modelo Nelson-Siegel", "12", "sec_yield"),
        ("MAIN", "7. Microestructura Cambiaria, Derivados Rofex y Fragilidad Sistémica", "13", "sec_fx"),
        ("SUB", "7.1. Tipo de Cambio Real Bilateral (TCR) y Competitividad Cambiaria", "14", "sec_tcr"),
        ("MAIN", "8. Sector Financiero, Renta Variable y Radar de Balances (Merval)", "15", "sec_equity"),
        ("CAT", "ANEXO & CIERRE", "", ""),
        ("MAIN", "9. Flash Normativo, Contexto Internacional y Referencias APA 7ma", "16", "sec_refs")
    ]
    ...
    # =========================================================================
    # PÁGINA 4: TABLERO INTEGRAL DE INDICADORES LÍDERES — ARQUETIPO TABLERO MASTER
    # =========================================================================
    elements.extend(crear_pagina_tablero_integral_macro(ctx))

    # =========================================================================
    # PÁGINA 5: 1. NIVEL DE ACTIVIDAD GENERAL (EMAE) — ARQUETIPO SCORECARD
    # =========================================================================
    p4_lead = (
    ...
    p4_tabla_data = [
        [Paragraph("<b>ACTIVIDAD / SECTOR</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>Trim.</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>PIB REAL Y DEMANDA AGREGADA</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>EMAE / PIB Total (% i.a.)</b>", table_cell_bold), Paragraph("1,80", table_cell_center_bold), Paragraph("2,40", table_cell_center_bold), Paragraph("2,80", table_cell_center_bold), Paragraph("3,10", table_cell_center_bold), _badge_var("+0,30"), _badge_var("+3,10%"), _badge_var("+3,10%"), Paragraph("3,50", table_cell_center_bold), Paragraph("4,20", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;Consumo Privado Hogares", table_cell_left), Paragraph("0,50", table_cell_center), Paragraph("1,80", table_cell_center), Paragraph("2,40", table_cell_center), Paragraph("2,70", table_cell_center), _badge_var("+0,30"), _badge_var("+2,70%"), _badge_var("+2,70%"), Paragraph("3,10", table_cell_center), Paragraph("3,80", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Consumo Público Nacional", table_cell_left), Paragraph("-5,20", table_cell_center), Paragraph("-3,10", table_cell_center), Paragraph("-1,50", table_cell_center), Paragraph("-0,90", table_cell_center), _badge_var("+0,60"), _badge_var("-0,90%"), _badge_var("-0,90%"), Paragraph("-0,50", table_cell_center), Paragraph("0,80", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Formación Bruta Capital Fijo", table_cell_left), Paragraph("-6,80", table_cell_center), Paragraph("2,40", table_cell_center), Paragraph("5,80", table_cell_center), Paragraph("6,40", table_cell_center), _badge_var("+0,60"), _badge_var("+6,40%"), _badge_var("+6,40%"), Paragraph("7,20", table_cell_center), Paragraph("8,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Exportaciones Reales de Bienes", table_cell_left), Paragraph("8,40", table_cell_center), Paragraph("9,50", table_cell_center), Paragraph("10,20", table_cell_center), Paragraph("11,40", table_cell_center), _badge_var("+1,20"), _badge_var("+11,4%"), _badge_var("+11,4%"), Paragraph("10,80", table_cell_center), Paragraph("7,50", table_cell_center)],
        [Paragraph("<b>TRACCIÓN SECTORIAL PRIMARIA & INDUSTRIAL</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Minería, Petróleo & Gas", table_cell_left), Paragraph("7,20", table_cell_center), Paragraph("8,10", table_cell_center), Paragraph("8,40", table_cell_center), Paragraph("8,50", table_cell_center), _badge_var("+0,10"), _badge_var("+8,50%"), _badge_var("+8,50%"), Paragraph("9,20", table_cell_center), Paragraph("11,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Sector Agropecuario", table_cell_left), Paragraph("10,50", table_cell_center), Paragraph("12,10", table_cell_center), Paragraph("13,40", table_cell_center), Paragraph("14,20", table_cell_center), _badge_var("+0,80"), _badge_var("+14,2%"), _badge_var("+14,2%"), Paragraph("12,50", table_cell_center), Paragraph("6,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Comercio Mayorista / Minorista", table_cell_left), Paragraph("-1,50", table_cell_center), Paragraph("0,40", table_cell_center), Paragraph("1,80", table_cell_center), Paragraph("2,80", table_cell_center), _badge_var("+1,00"), _badge_var("+2,80%"), _badge_var("+2,80%"), Paragraph("3,20", table_cell_center), Paragraph("4,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Industria Manufacturera", table_cell_left), Paragraph("-5,20", table_cell_center), Paragraph("-3,40", table_cell_center), Paragraph("-2,10", table_cell_center), Paragraph("-1,80", table_cell_center), _badge_var("+0,30"), _badge_var("-1,80%"), _badge_var("-1,80%"), Paragraph("0,50", table_cell_center), Paragraph("3,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Construcción & Cemento", table_cell_left), Paragraph("-10,50", table_cell_center), Paragraph("-7,20", table_cell_center), Paragraph("-5,10", table_cell_center), Paragraph("-4,20", table_cell_center), _badge_var("+0,90"), _badge_var("-4,20%"), _badge_var("-4,20%"), Paragraph("-1,00", table_cell_center), Paragraph("5,00", table_cell_center)],
    ]
    p4_bloques = [
    ...
    elements.extend(crear_pagina_arquetipo_scorecard(
        ...
        tabla_col_widths=[142, 40, 40, 40, 44, 45, 45, 46, 45, 45],
    ...
    # =========================================================================
    # PÁGINA 6: 2. PRECIOS Y SALARIOS (INDEC) — ARQUETIPO DESGLOSE
    # =========================================================================
    p5_lead = (
        "La inflación de Argentina profundizó su desaceleración en el segundo trimestre de 2026 (IPC general en 2,2% m/m y "
        "componente núcleo en 1,9% m/m), transmitiéndose hacia los precios mayoristas (IPIM general en 1,8% m/m e importados en 1,2% m/m), "
        "convalidando la eficacia del ancla fiscal en base caja y el crawling peg cambiario del 2%. "
        "En Mendoza, el IPC de la DEIE convergió al 2,3% m/m, mientras que los salarios formales (RIPTE) recomponen "
        "poder adquisitivo real frente a la Canasta Básica Alimentaria."
    )
    p5_tabla_data = [
        [Paragraph("<b>INDICADOR DE PRECIOS & INGRESOS</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>m/m</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>NIVEL GENERAL & APERTURAS INDEC</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>IPC General Nacional (% m/m)</b>", table_cell_bold), Paragraph("3,20", table_cell_center_bold), Paragraph("2,60", table_cell_center_bold), Paragraph("2,40", table_cell_center_bold), Paragraph("2,20", table_cell_center_bold), _badge_var("-0,20"), _badge_var("+29,5%"), _badge_var("+7,10%"), Paragraph("29,50", table_cell_center_bold), Paragraph("15,80", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;IPC Núcleo (Core Inflation)", table_cell_left), Paragraph("2,80", table_cell_center), Paragraph("2,20", table_cell_center), Paragraph("2,00", table_cell_center), Paragraph("1,90", table_cell_center), _badge_var("-0,10"), _badge_var("+24,8%"), _badge_var("+6,20%"), Paragraph("24,80", table_cell_center), Paragraph("12,40", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Precios Regulados (Tarifas)", table_cell_left), Paragraph("4,50", table_cell_center), Paragraph("3,80", table_cell_center), Paragraph("3,40", table_cell_center), Paragraph("3,00", table_cell_center), _badge_var("-0,40"), _badge_var("+38,2%"), _badge_var("+9,80%"), Paragraph("38,20", table_cell_center), Paragraph("18,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Precios Estacionales", table_cell_left), Paragraph("2,10", table_cell_center), Paragraph("1,80", table_cell_center), Paragraph("1,60", table_cell_center), Paragraph("1,40", table_cell_center), _badge_var("-0,20"), _badge_var("+20,5%"), _badge_var("+4,50%"), Paragraph("20,50", table_cell_center), Paragraph("14,00", table_cell_center)],
        [Paragraph("<b>PRECIOS MAYORISTAS & FORMACIÓN (IPIM)</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;IPIM Mayorista General (% m/m)", table_cell_left), Paragraph("2,60", table_cell_center), Paragraph("2,10", table_cell_center), Paragraph("1,90", table_cell_center), Paragraph("1,80", table_cell_center), _badge_var("-0,10"), _badge_var("+28,5%"), _badge_var("+5,80%"), Paragraph("28,50", table_cell_center), Paragraph("14,20", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;IPIM Bienes Importados (% m/m)", table_cell_left), Paragraph("1,90", table_cell_center), Paragraph("1,60", table_cell_center), Paragraph("1,40", table_cell_center), Paragraph("1,20", table_cell_center), _badge_var("-0,20"), _badge_var("+16,4%"), _badge_var("+3,90%"), Paragraph("16,40", table_cell_center), Paragraph("9,50", table_cell_center)],
        [Paragraph("<b>MEDICIÓN REGIONAL & SALARIOS</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;IPC DEIE Mendoza (% m/m)", table_cell_left), Paragraph("3,30", table_cell_center), Paragraph("2,70", table_cell_center), Paragraph("2,50", table_cell_center), Paragraph("2,30", table_cell_center), _badge_var("-0,20"), _badge_var("+30,2%"), _badge_var("+7,40%"), Paragraph("30,20", table_cell_center), Paragraph("16,20", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Salario Formal RIPTE (% i.a. nom.)", table_cell_left), Paragraph("112,4", table_cell_center), Paragraph("85,2", table_cell_center), Paragraph("62,4", table_cell_center), Paragraph("48,5", table_cell_center), _badge_var("-13,9"), _badge_var("+48,5%"), _badge_var("+12,4%"), Paragraph("34,20", table_cell_center), Paragraph("18,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Variación Salario Real (% trim.)", table_cell_left), Paragraph("-1,50", table_cell_center), Paragraph("0,80", table_cell_center), Paragraph("1,40", table_cell_center), Paragraph("1,80", table_cell_center), _badge_var("+0,40"), _badge_var("+4,80%"), _badge_var("+1,80%"), Paragraph("2,80", table_cell_center), Paragraph("3,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Tasa Real Ex-Ante Contractual", table_cell_left), Paragraph("0,45", table_cell_center), Paragraph("0,65", table_cell_center), Paragraph("0,85", table_cell_center), Paragraph("0,95", table_cell_center), _badge_var("+0,10"), _badge_var("+0,70"), _badge_var("+0,95"), Paragraph("1,05", table_cell_center), Paragraph("0,80", table_cell_center)],
    ]
    p5_bullets = [
    ...
    p6_tabla_data = [
        [Paragraph("<b>CANASTA / UMBRAL DE POBREZA</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>m/m</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>VALORIZACIÓN DE CANASTAS (MILES ARS)</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>CBT Nacional (Línea Pobreza)</b>", table_cell_bold), Paragraph("910,2", table_cell_center_bold), Paragraph("980,5", table_cell_center_bold), Paragraph("1.045", table_cell_center_bold), Paragraph("1.175", table_cell_center_bold), _badge_var("+2,20%"), _badge_var("+43,2%"), _badge_var("+12,4%"), Paragraph("1.250", table_cell_center_bold), Paragraph("1.420", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;CBA Nacional (Línea Indigencia)", table_cell_left), Paragraph("412,0", table_cell_center), Paragraph("445,2", table_cell_center), Paragraph("478,0", table_cell_center), Paragraph("532,0", table_cell_center), _badge_var("+2,10%"), _badge_var("+43,6%"), _badge_var("+11,3%"), Paragraph("565,0", table_cell_center), Paragraph("640,0", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;CBT Mendoza (DEIE Cuyo)", table_cell_left), Paragraph("845,0", table_cell_center), Paragraph("912,0", table_cell_center), Paragraph("963,0", table_cell_center), Paragraph("1.085", table_cell_center), _badge_var("+2,30%"), _badge_var("+41,8%"), _badge_var("+12,7%"), Paragraph("1.160", table_cell_center), Paragraph("1.310", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;CBA Mendoza (DEIE Cuyo)", table_cell_left), Paragraph("380,0", table_cell_center), Paragraph("410,0", table_cell_center), Paragraph("435,0", table_cell_center), Paragraph("485,0", table_cell_center), _badge_var("+2,10%"), _badge_var("+40,6%"), _badge_var("+11,5%"), Paragraph("515,0", table_cell_center), Paragraph("585,0", table_cell_center)],
        [Paragraph("<b>INDICADORES SOCIALES & COBERTURA</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Brecha Salario RIPTE / CBT (%)", table_cell_left), Paragraph("108,2", table_cell_center), Paragraph("112,0", table_cell_center), Paragraph("115,4", table_cell_center), Paragraph("118,5", table_cell_center), _badge_var("+3,10 pp"), _badge_var("+13,1 pp"), _badge_var("+3,10 pp"), Paragraph("122,0", table_cell_center), Paragraph("128,0", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Tasa de Pobreza Urbana (% est.)", table_cell_left), Paragraph("51,80", table_cell_center), Paragraph("48,50", table_cell_center), Paragraph("46,20", table_cell_center), Paragraph("44,10", table_cell_center), _badge_var("-2,10 pp"), _badge_var("-10,1 pp"), _badge_var("-2,10 pp"), Paragraph("41,50", table_cell_center), Paragraph("36,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Tasa de Indigencia (% est.)", table_cell_left), Paragraph("16,20", table_cell_center), Paragraph("14,10", table_cell_center), Paragraph("12,80", table_cell_center), Paragraph("11,40", table_cell_center), _badge_var("-1,40 pp"), _badge_var("-7,10 pp"), _badge_var("-1,40 pp"), Paragraph("10,00", table_cell_center), Paragraph("8,20", table_cell_center)],
    ]
    p6_bullets = [
    ...
    p7_tabla_data = [
        [Paragraph("<b>CADENA PRODUCTIVA / SECTOR</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>Trim.</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>VITIVINICULTURA & AGROINDUSTRIA</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>Vino Fraccionado (Miles hl)</b>", table_cell_bold), Paragraph("6.850", table_cell_center_bold), Paragraph("7.120", table_cell_center_bold), Paragraph("7.250", table_cell_center_bold), Paragraph("7.340", table_cell_center_bold), _badge_var("+1,24%"), _badge_var("+18,4%"), _badge_var("+1,24%"), Paragraph("7.500", table_cell_center_bold), Paragraph("7.850", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;Despacho Mercado Interno (%)", table_cell_left), Paragraph("-1,80", table_cell_center), Paragraph("0,50", table_cell_center), Paragraph("1,80", table_cell_center), Paragraph("2,80", table_cell_center), _badge_var("+1,00"), _badge_var("+2,80%"), _badge_var("+2,80%"), Paragraph("3,20", table_cell_center), Paragraph("4,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Exportaciones Vino Fraccionado (%)", table_cell_left), Paragraph("2,80", table_cell_center), Paragraph("3,40", table_cell_center), Paragraph("4,10", table_cell_center), Paragraph("4,80", table_cell_center), _badge_var("+0,70"), _badge_var("+4,80%"), _badge_var("+4,80%"), Paragraph("5,50", table_cell_center), Paragraph("6,80", table_cell_center)],
        [Paragraph("<b>HIDROCARBUROS & CEMENTO PORTLAND</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Petróleo Total Mendoza (Miles m³)", table_cell_left), Paragraph("208,5", table_cell_center), Paragraph("210,4", table_cell_center), Paragraph("211,8", table_cell_center), Paragraph("212,0", table_cell_center), _badge_var("+0,09%"), _badge_var("+3,41%"), _badge_var("+0,09%"), Paragraph("215,0", table_cell_center), Paragraph("224,0", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Vaca Muerta Sur Mendoza (Shale)", table_cell_left), Paragraph("22,00", table_cell_center), Paragraph("25,40", table_cell_center), Paragraph("28,10", table_cell_center), Paragraph("30,00", table_cell_center), _badge_var("+6,76%"), _badge_var("+62,2%"), _badge_var("+6,76%"), Paragraph("34,00", table_cell_center), Paragraph("45,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Cuenca Cuyana Convencional (%)", table_cell_left), Paragraph("-2,40", table_cell_center), Paragraph("-1,80", table_cell_center), Paragraph("-1,20", table_cell_center), Paragraph("-0,80", table_cell_center), _badge_var("+0,40"), _badge_var("-0,80%"), _badge_var("-0,80%"), Paragraph("-0,50", table_cell_center), Paragraph("0,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Despacho Cemento Cuyo (AFCP %)", table_cell_left), Paragraph("-8,50", table_cell_center), Paragraph("-4,20", table_cell_center), Paragraph("-1,50", table_cell_center), Paragraph("1,20", table_cell_center), _badge_var("+2,70"), _badge_var("+1,20%"), _badge_var("+1,20%"), Paragraph("3,50", table_cell_center), Paragraph("7,80", table_cell_center)],
    ]
    p7_bullets = [
    ...
    p8_tabla_data = [
        [Paragraph("<b>JURISDICCIÓN / INDICADOR ISARC</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>Trim.</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>ÍNDICE REGIONAL CUYO & PROVINCIAS</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>ISARC Región Cuyo (% i.a.)</b>", table_cell_bold), Paragraph("2,10", table_cell_center_bold), Paragraph("2,60", table_cell_center_bold), Paragraph("2,80", table_cell_center_bold), Paragraph("3,10", table_cell_center_bold), _badge_var("+0,30"), _badge_var("+3,10%"), _badge_var("+3,10%"), Paragraph("3,40", table_cell_center_bold), Paragraph("4,50", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;Mendoza (Petróleo / Vitivinicultura)", table_cell_left), Paragraph("2,20", table_cell_center), Paragraph("2,50", table_cell_center), Paragraph("3,10", table_cell_center), Paragraph("3,40", table_cell_center), _badge_var("+0,30"), _badge_var("+3,40%"), _badge_var("+3,40%"), Paragraph("3,60", table_cell_center), Paragraph("4,60", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;San Luis (Manufacturas / Alimentos)", table_cell_left), Paragraph("4,00", table_cell_center), Paragraph("4,80", table_cell_center), Paragraph("5,20", table_cell_center), Paragraph("5,80", table_cell_center), _badge_var("+0,60"), _badge_var("+5,80%"), _badge_var("+5,80%"), Paragraph("6,20", table_cell_center), Paragraph("5,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;San Juan (Minería / Agroindustria)", table_cell_left), Paragraph("1,50", table_cell_center), Paragraph("1,90", table_cell_center), Paragraph("2,00", table_cell_center), Paragraph("2,10", table_cell_center), _badge_var("+0,10"), _badge_var("+2,10%"), _badge_var("+2,10%"), Paragraph("2,80", table_cell_center), Paragraph("5,20", table_cell_center)],
        [Paragraph("<b>TRACCIÓN SECTORIAL REGIONAL</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Minería Metalífera & Hidrocarburos", table_cell_left), Paragraph("6,50", table_cell_center), Paragraph("7,20", table_cell_center), Paragraph("8,10", table_cell_center), Paragraph("8,50", table_cell_center), _badge_var("+0,40"), _badge_var("+8,50%"), _badge_var("+8,50%"), Paragraph("9,50", table_cell_center), Paragraph("12,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Agroindustria Alimentaria Cuyo", table_cell_left), Paragraph("2,80", table_cell_center), Paragraph("3,40", table_cell_center), Paragraph("3,60", table_cell_center), Paragraph("3,80", table_cell_center), _badge_var("+0,20"), _badge_var("+3,80%"), _badge_var("+3,80%"), Paragraph("4,20", table_cell_center), Paragraph("4,80", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Comercio & Servicios Cuyanos", table_cell_left), Paragraph("0,20", table_cell_center), Paragraph("1,50", table_cell_center), Paragraph("2,10", table_cell_center), Paragraph("2,40", table_cell_center), _badge_var("+0,30"), _badge_var("+2,40%"), _badge_var("+2,40%"), Paragraph("2,80", table_cell_center), Paragraph("3,60", table_cell_center)],
    ]
    p8_bloques = [
    ...
    p9_tabla_data = [
        [Paragraph("<b>VARIABLE DE BALANCE BCRA</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>Trim.</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>AGREGADOS MONETARIOS (BILLONES ARS)</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>Base Monetaria (Promedio)</b>", table_cell_bold), Paragraph("21,50", table_cell_center_bold), Paragraph("23,40", table_cell_center_bold), Paragraph("24,80", table_cell_center_bold), Paragraph("26,80", table_cell_center_bold), _badge_var("+2,00"), _badge_var("+35,4%"), _badge_var("+8,06%"), Paragraph("28,50", table_cell_center_bold), Paragraph("32,00", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;Circulante en Poder Público", table_cell_left), Paragraph("13,80", table_cell_center), Paragraph("14,90", table_cell_center), Paragraph("15,80", table_cell_center), Paragraph("16,90", table_cell_center), _badge_var("+1,10"), _badge_var("+36,3%"), _badge_var("+6,96%"), Paragraph("18,00", table_cell_center), Paragraph("20,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Encajes Bancarios en BCRA", table_cell_left), Paragraph("7,70", table_cell_center), Paragraph("8,50", table_cell_center), Paragraph("9,00", table_cell_center), Paragraph("9,90", table_cell_center), _badge_var("+0,90"), _badge_var("+33,8%"), _badge_var("+10,0%"), Paragraph("10,50", table_cell_center), Paragraph("11,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Stock Pases Remunerados", table_cell_left), Paragraph("1,50", table_cell_center), Paragraph("0,00", table_cell_center), Paragraph("0,00", table_cell_center), Paragraph("0,00", table_cell_center), _badge_var("0,00"), _badge_var("-100%"), _badge_var("0,00"), Paragraph("0,00", table_cell_center), Paragraph("0,00", table_cell_center)],
        [Paragraph("<b>RESERVAS & TASAS DE INTERÉS</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Reservas Brutas (USD Millones)", table_cell_left), Paragraph("29.450", table_cell_center), Paragraph("31.200", table_cell_center), Paragraph("32.800", table_cell_center), Paragraph("34.150", table_cell_center), _badge_var("+1.350"), _badge_var("+25,9%"), _badge_var("+4,12%"), Paragraph("36.000", table_cell_center), Paragraph("41.500", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Reservas Netas (RIN USD MM)", table_cell_left), Paragraph("-1.100", table_cell_center), Paragraph("+850", table_cell_center), Paragraph("+2.400", table_cell_center), Paragraph("+3.650", table_cell_center), _badge_var("+1.250"), _badge_var("+6.850"), _badge_var("+1.250"), Paragraph("+5.000", table_cell_center), Paragraph("+8.500", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Tasa Política Monetaria (TNA %)", table_cell_left), Paragraph("40,00", table_cell_center), Paragraph("35,00", table_cell_center), Paragraph("35,00", table_cell_center), Paragraph("32,00", table_cell_center), _badge_var("-3,00"), _badge_var("-13,0"), _badge_var("-3,00"), Paragraph("30,00", table_cell_center), Paragraph("25,00", table_cell_center)],
    ]
    p9_bullets = [
    ...
    p11_tabla_data = [
        [Paragraph("<b>PARÁMETRO / BONO SOBERANO</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>Trim.</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>PARÁMETROS NELSON-SIEGEL (1987)</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>Nivel Asintótico Largo Plazo (Beta 0 %)</b>", table_cell_bold), Paragraph("12,50", table_cell_center_bold), Paragraph("11,20", table_cell_center_bold), Paragraph("10,10", table_cell_center_bold), Paragraph("9,40", table_cell_center_bold), _badge_var("-0,70"), _badge_var("-4,80"), _badge_var("-0,70"), Paragraph("8,80", table_cell_center_bold), Paragraph("7,50", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;Pendiente Corto-Largo (Beta 1 %)", table_cell_left), Paragraph("9,80", table_cell_center), Paragraph("7,50", table_cell_center), Paragraph("6,20", table_cell_center), Paragraph("5,60", table_cell_center), _badge_var("-0,60"), _badge_var("-6,80"), _badge_var("-0,60"), Paragraph("4,20", table_cell_center), Paragraph("2,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Curvatura Joroba Media (Beta 2 %)", table_cell_left), Paragraph("-6,20", table_cell_center), Paragraph("-4,80", table_cell_center), Paragraph("-3,90", table_cell_center), Paragraph("-3,20", table_cell_center), _badge_var("+0,70"), _badge_var("+5,30"), _badge_var("+0,70"), Paragraph("-2,40", table_cell_center), Paragraph("-1,20", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Bondad de Ajuste Modelo (R²)", table_cell_left), Paragraph("0,962", table_cell_center), Paragraph("0,974", table_cell_center), Paragraph("0,980", table_cell_center), Paragraph("0,984", table_cell_center), _badge_var("+0,004"), _badge_var("+0,039"), _badge_var("+0,004"), Paragraph("0,988", table_cell_center), Paragraph("0,992", table_cell_center)],
        [Paragraph("<b>RENDIMIENTOS DE MERCADO (TIR %)</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Bonar 2030 (AL30 Ley Local)", table_cell_left), Paragraph("15,20", table_cell_center), Paragraph("13,40", table_cell_center), Paragraph("12,10", table_cell_center), Paragraph("11,20", table_cell_center), _badge_var("-0,90"), _badge_var("-7,30"), _badge_var("-0,90"), Paragraph("10,00", table_cell_center), Paragraph("8,20", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Global 2035 (GD35 Ley NY)", table_cell_left), Paragraph("12,90", table_cell_center), Paragraph("11,50", table_cell_center), Paragraph("10,40", table_cell_center), Paragraph("9,65", table_cell_center), _badge_var("-0,75"), _badge_var("-5,15"), _badge_var("-0,75"), Paragraph("8,90", table_cell_center), Paragraph("7,60", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Global 2038 (GD38 Ley NY)", table_cell_left), Paragraph("13,10", table_cell_center), Paragraph("11,70", table_cell_center), Paragraph("10,50", table_cell_center), Paragraph("9,70", table_cell_center), _badge_var("-0,80"), _badge_var("-5,40"), _badge_var("-0,80"), Paragraph("9,00", table_cell_center), Paragraph("7,80", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Riesgo País EMBI+ (Puntos Básicos)", table_cell_left), Paragraph("980", table_cell_center), Paragraph("810", table_cell_center), Paragraph("680", table_cell_center), Paragraph("506", table_cell_center), _badge_var("-174"), _badge_var("-734"), _badge_var("-174"), Paragraph("450", table_cell_center), Paragraph("320", table_cell_center)],
    ]
    p11_conclusiones = [
    ...
    # =========================================================================
    # PÁGINA 13: 7. MICROESTRUCTURA FX Y ROFEX CIP — ARQUETIPO ASIMÉTRICO
    # =========================================================================
    p12_lead = (
        ...
        "mientras que el barómetro de confianza en el gobierno (ICG en 2,48 pts) y del consumidor (ICC en 41,8 pts) ratifican estabilidad."
    )
    p12_bullets_asym = [
    ...
    # =========================================================================
    # PÁGINA 14: 7.1. TIPO DE CAMBIO REAL BILATERAL — ARQUETIPO DESGLOSE
    # =========================================================================
    p13_lead = (
        "El Tipo de Cambio Real Bilateral (TCR) con Estados Unidos finalizó en 78,4 puntos (base dic-2016 = 100), "
        "mientras que el multilateral TCRM se ubicó en 94,5 puntos y con Brasil en 102,4 puntos. "
        "La solvencia externa descansa en la solidez de la Balanza de Pagos: la Cuenta Corriente sostiene un saldo positivo "
        "de USD 1.840 M trimestral y la balanza comercial de bienes FOB aportó USD 3.250 M."
    )
    p13_tabla_data = [
        [Paragraph("<b>INDICADOR DE PRECIOS RELATIVOS / TCR</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>Trim.</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>TIPO DE CAMBIO REAL & COMPETITIVIDAD</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>TCR Bilateral ARS/USD (Base 100)</b>", table_cell_bold), Paragraph("81,20", table_cell_center_bold), Paragraph("80,50", table_cell_center_bold), Paragraph("79,50", table_cell_center_bold), Paragraph("78,40", table_cell_center_bold), _badge_var("-1,10"), _badge_var("-5,60"), _badge_var("-1,10"), Paragraph("76,50", table_cell_center_bold), Paragraph("75,00", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;TCR Multilateral BCRA (TCRM)", table_cell_left), Paragraph("87,40", table_cell_center), Paragraph("86,20", table_cell_center), Paragraph("85,80", table_cell_center), Paragraph("94,50", table_cell_center), _badge_var("+8,70"), _badge_var("+5,00"), _badge_var("+8,70"), Paragraph("92,00", table_cell_center), Paragraph("90,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;TCR Bilateral con Brasil (Real/ARS)", table_cell_left), Paragraph("90,10", table_cell_center), Paragraph("88,50", table_cell_center), Paragraph("87,40", table_cell_center), Paragraph("102,4", table_cell_center), _badge_var("+15,0"), _badge_var("+10,0"), _badge_var("+15,0"), Paragraph("98,00", table_cell_center), Paragraph("95,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Crawling Peg Mensual BCRA (%)", table_cell_left), Paragraph("2,00", table_cell_center), Paragraph("2,00", table_cell_center), Paragraph("2,00", table_cell_center), Paragraph("2,00", table_cell_center), _badge_var("0,00"), _badge_var("0,00"), _badge_var("0,00"), Paragraph("2,00", table_cell_center), Paragraph("1,50", table_cell_center)],
        [Paragraph("<b>BALANZA DE PAGOS & CUENTAS EXTERNAS (USD MM)</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>Cuenta Corriente Balanza Pagos</b>", table_cell_bold), Paragraph("+980", table_cell_center_bold), Paragraph("+1.240", table_cell_center_bold), Paragraph("+1.520", table_cell_center_bold), Paragraph("+1.840", table_cell_center_bold), _badge_var("+320"), _badge_var("+1.190"), _badge_var("+320"), Paragraph("+6.800", table_cell_center_bold), Paragraph("+8.500", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;Balanza Comercial Bienes FOB", table_cell_left), Paragraph("3.120", table_cell_center), Paragraph("2.950", table_cell_center), Paragraph("3.100", table_cell_center), Paragraph("3.250", table_cell_center), _badge_var("+150"), _badge_var("+400"), _badge_var("+150"), Paragraph("13.200", table_cell_center), Paragraph("15.800", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Exportaciones de Bienes FOB", table_cell_left), Paragraph("6.850", table_cell_center), Paragraph("6.950", table_cell_center), Paragraph("7.050", table_cell_center), Paragraph("7.120", table_cell_center), _badge_var("+70"), _badge_var("+870"), _badge_var("+70"), Paragraph("29.500", table_cell_center), Paragraph("32.500", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Importaciones de Bienes CIF", table_cell_left), Paragraph("3.730", table_cell_center), Paragraph("4.000", table_cell_center), Paragraph("3.950", table_cell_center), Paragraph("3.870", table_cell_center), _badge_var("-80"), _badge_var("+470"), _badge_var("-80"), Paragraph("16.300", table_cell_center), Paragraph("16.700", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Cuenta Financiera Neta Cambiaria", table_cell_left), Paragraph("+650", table_cell_center), Paragraph("+810", table_cell_center), Paragraph("+950", table_cell_center), Paragraph("+1.150", table_cell_center), _badge_var("+200"), _badge_var("+730"), _badge_var("+200"), Paragraph("+4.200", table_cell_center), Paragraph("+6.000", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Variación RIN BCRA (Liquidación)", table_cell_left), Paragraph("+1.850", table_cell_center), Paragraph("+2.400", table_cell_center), Paragraph("+3.100", table_cell_center), Paragraph("+3.650", table_cell_center), _badge_var("+550"), _badge_var("+2.500"), _badge_var("+550"), Paragraph("+14.850", table_cell_center), Paragraph("+18.000", table_cell_center)],
    ]
    p13_bullets = [
    ...
    p14_tabla_data = [
        [Paragraph("<b>EMPRESA LÍDER / RATIO BYMA</b>", table_header_style), Paragraph("<b>2T25</b>", table_header_style), Paragraph("<b>3T25</b>", table_header_style), Paragraph("<b>4T25</b>", table_header_style), Paragraph("<b>1T26</b>", table_header_style), Paragraph("<b>Var.<br/>Trim.</b>", table_header_style), Paragraph("<b>Var.<br/>i.a.</b>", table_header_style), Paragraph("<b>Var.<br/>Acum.</b>", table_header_style), Paragraph("<b>2026e</b>", table_header_style), Paragraph("<b>2027e</b>", table_header_style)],
        [Paragraph("<b>ÍNDICE S&P MERVAL Y RATIOS GENERALES</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("<b>Merval en USD CCL (Puntos)</b>", table_cell_bold), Paragraph("1.580", table_cell_center_bold), Paragraph("1.720", table_cell_center_bold), Paragraph("1.820", table_cell_center_bold), Paragraph("1.905", table_cell_center_bold), _badge_var("+4,67%"), _badge_var("+34,2%"), _badge_var("+4,67%"), Paragraph("2.100", table_cell_center_bold), Paragraph("2.400", table_cell_center_bold)],
        [Paragraph("&nbsp;&nbsp;Volumen Operado ByMA (USD MM día)", table_cell_left), Paragraph("42,00", table_cell_center), Paragraph("48,50", table_cell_center), Paragraph("52,40", table_cell_center), Paragraph("58,00", table_cell_center), _badge_var("+5,60"), _badge_var("+64,8%"), _badge_var("+10,7%"), Paragraph("65,00", table_cell_center), Paragraph("85,00", table_cell_center)],
        [Paragraph("<b>VALUACIONES DE ACCIONES LÍDERES</b>", table_cell_subhdr), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center), Paragraph("", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;YPF S.A. (EV/EBITDA x)", table_cell_left), Paragraph("4,40", table_cell_center), Paragraph("4,10", table_cell_center), Paragraph("3,90", table_cell_center), Paragraph("3,80", table_cell_center), _badge_var("-0,10"), _badge_var("-1,00"), _badge_var("-0,10"), Paragraph("3,50", table_cell_center), Paragraph("3,20", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;YPF S.A. (Margen EBITDA %)", table_cell_left), Paragraph("30,10", table_cell_center), Paragraph("31,40", table_cell_center), Paragraph("32,00", table_cell_center), Paragraph("32,40", table_cell_center), _badge_var("+0,40"), _badge_var("+3,90"), _badge_var("+0,40"), Paragraph("34,00", table_cell_center), Paragraph("36,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Pampa Energía (EV/EBITDA x)", table_cell_left), Paragraph("4,70", table_cell_center), Paragraph("4,40", table_cell_center), Paragraph("4,20", table_cell_center), Paragraph("4,10", table_cell_center), _badge_var("-0,10"), _badge_var("-1,00"), _badge_var("-0,10"), Paragraph("3,80", table_cell_center), Paragraph("3,50", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Pampa Energía (Margen EBITDA %)", table_cell_left), Paragraph("36,00", table_cell_center), Paragraph("37,50", table_cell_center), Paragraph("38,00", table_cell_center), Paragraph("38,50", table_cell_center), _badge_var("+0,50"), _badge_var("+4,30"), _badge_var("+0,50"), Paragraph("40,00", table_cell_center), Paragraph("42,00", table_cell_center)],
        [Paragraph("&nbsp;&nbsp;Grupo Financiero Galicia (EV/EBITDA)", table_cell_left), Paragraph("7,20", table_cell_center), Paragraph("6,80", table_cell_center), Paragraph("6,40", table_cell_center), Paragraph("6,20", table_cell_center), _badge_var("-0,20"), _badge_var("-1,60"), _badge_var("-0,20"), Paragraph("5,80", table_cell_center), Paragraph("5,00", table_cell_center)],
    ]
    p14_conclusiones = [
